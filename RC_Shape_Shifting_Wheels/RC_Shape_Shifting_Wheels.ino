...
void state_excution()
{
  if(state == 0) //Lock valve nothing happen
  {
    analogWrite(E1,0); //Pump off
    delay(50);//delay for valve
    analogWrite(E2,0); //Valve closed
  }
  else if(state == 1)//Inflating
  {
    analogWrite(E1,power); //Pump on
    delay(50);//delay for valve
    analogWrite(E2,0); //Valve closed
  }
...
void setup() 
{
  ...
  attachInterrupt(digitalPinToInterrupt(channel3_PIN), rising1, RISING);
...
void staging()
{
  ...
  //Check if c1 is above starting power threshold percentage
  if(abs(c1)< starting_power_threshold)c1 = 0; 

  if (c1 >0)state == 1; // inflate mode
  else if (c1== 0) state == 0; //Lock mode
  else if (c1<0) state == 2;//deflate mode

  power = MAX_POWER*c1/100;
...
void rising1() 
{
  attachInterrupt(digitalPinToInterrupt(channel3_PIN), falling1, FALLING);
  prev_time1 = micros();
}
void falling1() 
{
...
void print_status(int print_every_x_millis)
{
  ...
  {
    Serial.print("Channel 3 PWM value(LR): ");
